...
##           By: 
##     Update #: 
# Comments: This is a modified version of the survSuperLearner by Westling et al.
# the main function has a few modifications: 

# first, the computation of the 
# cross validated base library risks in addition to event and censoring predictions is
# done in parallel

# second, the base function automatically calculates predictions for the fitted data set
# which can take a very long time, and is not always needed. Here we add a simple "predict"
# input option to determine if we want to predict for the base data set we feed into the 
# function or not
#----------------------------------------------------------------------
...
ParSurvSuperLearner <- function(time, event, X, newX = NULL, new.times, 
                             event.SL.library, cens.SL.library, id = NULL,
                             verbose = FALSE, control = list(), 
                             cvControl = list(), obsWeights = NULL, predict = FALSE, cluster = NULL)  {
  require(survSuperLearner)
  require(parallel)
  
  
  # Check if cluster input is proper
  cluster_classCheck <- class(cluster) == c("SOCKcluster","cluster")
  if(! all(cluster_classCheck)){
    stop("Please input a valid cluster, by using the parallel::makeCluster()")
  }
  
  if(!is.logical(predict)){
    stop("predict must be logical (T/F)")
  }
  
  # clusterExport(cluster, c("survSuperLearner.control","survSuperLearner.CV.control"))
  clusterEvalQ(cl, library(survSuperLearner))
  
  # Check to see if required packages are installed
  packages <- c("survival", "mgcv", "randomForestSRC", "glmnet", "SuperLearner",
                "nnls", "Rsolnp", "parallel")
  for (pkg in packages) {
  ...
  if(verbose) message("Getting cross-validated survivals for S and T")
  ...
  event.crossValFUN_out <- parLapply(cl = cluster, X = validRows, fun = .crossValFUN,
  ...
  event.errorsInCVLibrary <- parApply(cl = cluster, X = event.Z, MARGIN = 3, FUN = anyNA)
  ...
  cens.crossValFUN_out <- parLapply(cl = cluster, X = validRows,fun = .crossValFUN,
  ...
  cens.errorsInCVLibrary <- parApply(cl = cluster, X = cens.Z, MARGIN = 3, FUN = anyNA)
  ...
  if(verbose) message("Performing iterative SuperLearning to get coefficients")
  ...
  if(predict == T){
    ...
    if(verbose) message("Getting library and SL predictions")
  ...
  event.whichScreen <- parSapply(cl = cluster, X = event.library$screenAlgorithm, FUN = .screenFun,
                                 list = list(time = time, event = event, id = id, X = X, obsWeights = obsWeights),
                                 simplify = FALSE)
  event.whichScreen <- do.call(rbind, event.whichScreen)
  
  cens.whichScreen <- parSapply(cl = cluster, X = cens.library$screenAlgorithm, FUN = .screenFun,
  ...
  event.pred <- parLapply(cl = cluster, X = seq(event.k), fun = .predFun,
  ...
  event.errorsInLibrary <- parApply(cl = cluster, X = event.libraryPred, MARGIN = 3, 
                                    function(algorithm) anyNA(algorithm))
  ...
  cens.pred <- parLapply(cl = cluster, X = seq(cens.k), fun = .predFun,
  ...
  cens.errorsInLibrary <- parApply(cl = cluster, X = cens.libraryPred, MARGIN = 3,
                                   function(algorithm) anyNA(algorithm))
  ...
  class(out) <- c("ParsurvSuperLearner")
  return(out)
}
...
predict.ParsurvSuperLearner <- function (object, newdata, new.times, X = NULL, time = NULL, event = NULL, onlySL = FALSE, threshold = 1e-4, ...) {
  if (missing(newdata)) {
...
trainSurv <- riskRegression::sampleData(1000,outcome="survival")[,-c("eventtime","censtime")]
testSurv <- riskRegression::sampleData(200,outcome="survival")[,-c("eventtime","censtime")]


lib <- c("survSL.km","survSL.weibreg","survSL.coxph","survSL.rfsrc")
times <- c(5,8)
X <- trainSurv[,-c("time","event")]
X2 <- testSurv[,-c("time","event")]


# without predictions
system.time(
  
  fit_ssl_nopred <- ParSurvSuperLearner(time = trainSurv$time,
                                        event = trainSurv$event,
                                        event.SL.library = lib,
                                        cens.SL.library = lib,
                                        new.times = times, X = data.frame(X), 
                                        cluster = cl, predict = F, verbose = T)
  
)
fit_ssl_nopred$event.coef

# with predictions
system.time(
  
  fit_ssl <- ParSurvSuperLearner(time = trainSurv$time,
                                 event = trainSurv$event,
                                 event.SL.library = lib,
                                 cens.SL.library = lib,
                                 new.times = times, X=data.frame(X), 
                                 cluster = cl, predict = T, verbose = T)
  
)
fit_ssl$event.coef

# the parallel implementation is a lot slower at a sample size of 1000
# at a sample size of 10000, parallel is faster
